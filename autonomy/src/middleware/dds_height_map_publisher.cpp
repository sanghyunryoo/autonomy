...
void DdsHeightMapPublisher::publish(const std::vector<float> & data)
{
  if (writer_ <= 0) {
    return;
  }
  ...
  dds_sample.data._maximum = static_cast<std::uint32_t>(data.size());
  dds_sample.data._length = static_cast<std::uint32_t>(data.size());
  dds_sample.data._buffer = const_cast<float *>(data.data());
